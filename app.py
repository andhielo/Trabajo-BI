...
# PALETA DE COLORES ESTÉTICA (Temática Salud y Mente)
COLOR_PRIMARY = "#1E3A8A"   # Azul Marino Profundo
COLOR_SUCCESS = "#059669"   # Verde Esmeralda (Soporte / Bajo Riesgo)
COLOR_WARNING = "#D97706"   # Ámbar Cálido (Moderado)
COLOR_DANGER = "#E11D48"    # Coral/Rojo Suave (Alerta Crítica)
COLOR_MUTED = "#64748B"     # Gris Slate

# ==========================================
# ESTILOS CSS PERSONALIZADOS (TEMA BLANCO ENHANCED)
# ==========================================
st.markdown(f"""
    <style>
    ...
    [data-testid="baseButton-primary"]:hover {{
        background-color: #172554 !important; /* Azul más oscuro al pasar mouse */
    }}
    ...
    /* Ajuste de los Labels para que siempre resalten en negro */
    label {{
...
if st.session_state.current_page == "Pantalla General":
    ...
    with col3:
        with st.container(border=True):
            ...
            st.markdown(f'<p class="step-status" style="color: {COLOR_SUCCESS};">Transformación fluida</p>', unsafe_allow_html=True)
    ...
    with col4:
        with st.container(border=True):
            ...
            st.markdown(f'<p class="step-status" style="color: {COLOR_WARNING};">Esquema estrella/copo</p>', unsafe_allow_html=True)
    ...
    with col5:
        with st.container(border=True):
            ...
            st.markdown(f'<p class="step-status" style="color: {COLOR_DANGER};">Prevención con sesgo cero</p>', unsafe_allow_html=True)
    ...
    with col6:
        with st.container(border=True):
            ...
            st.markdown(f'<p class="step-status" style="color: {COLOR_PRIMARY};">Negocio y analítica</p>', unsafe_allow_html=True)
...
# --- PASO 2: STAGING AREA ---
    ...
    with c_m3:
        st.markdown(f'<div class="metric-label">Errores Sanitizados</div><div class="metric-value" style="color: {COLOR_SUCCESS};">0</div>', unsafe_allow_html=True)
...
# --- PASO 5: CAPA DE IA ÉTICA ---
    ...
    with col_ia1:
        fig_prob = px.histogram(df_pred, x="Prob_Riesgo", color="Riesgo_Predicho_IA", 
                                ...
                                color_discrete_map={"Riesgo Alto": COLOR_DANGER, "Riesgo Moderado": COLOR_WARNING, "Riesgo Bajo": COLOR_SUCCESS},
                                labels={"Prob_Riesgo": "Índice de Riesgo (0-1)", "Riesgo_Predicho_IA": "Riesgo Asignado"})
        fig_prob.update_layout(
            plot_bgcolor="white", 
            paper_bgcolor="white",
            font=dict(color="#1E293B", size=11),
            title_font=dict(color=COLOR_PRIMARY, size=14, family="Arial")
        )
    ...
    with col_ia2:
        fig_scatter = px.scatter(df_pred, x="Horas_Sueno_Promedio", y="Estres_Academico", color="Riesgo_Predicho_IA",
                                 ...
                                 color_discrete_map={"Riesgo Alto": COLOR_DANGER, "Riesgo Moderado": COLOR_WARNING, "Riesgo Bajo": COLOR_SUCCESS},
                                 labels={"Horas_Sueno_Promedio": "Horas de Sueño Promedio", "Estres_Academico": "Nivel de Estrés Académico (1-10)", "Riesgo_Predicho_IA": "Riesgo"})
...
# --- PASO 7: VISUALIZACIÓN BI ---
    ...
    st.button("← Volver a la Pantalla General", on_click=set_page, args=("Pantalla General",))
    st.markdown("<h2 style='color: #0F172A; margin-bottom: 30px;'>Visualización BI - Dashboard Directivo</h2>", unsafe_allow_html=True)

    col_bi_f1, col_bi_f2 = st.columns([1, 4])
    with col_bi_f1:
        st.markdown("<p style='font-weight:bold; color:#1E293B;'>Filtros Rápidos</p>", unsafe_allow_html=True)
    ...
    with col_bi_f2:
        # Gráfica 1: Nivel de Ansiedad por Distrito (CON COLOR ACENTUADO AZUL MARINO)
        ...
        fig_bar = px.bar(dist_ans, x='Distrito_Residencia', y='Puntuacion_Ansiedad_GAD7', 
                         title="Nivel de Ansiedad GAD-7 Promedio por Distrito",
                         color_discrete_sequence=[COLOR_PRIMARY],
                         labels={"Distrito_Residencia": "Distrito de Residencia", "Puntuacion_Ansiedad_GAD7": "Puntuación Promedio GAD-7"})
        fig_bar.update_layout(
            plot_bgcolor="white", 
            paper_bgcolor="white",
            font=dict(color="#1E293B", size=11),  # Texto legible y contrastado
            title_font=dict(color=COLOR_PRIMARY, size=15, family="Arial", weight="bold"),
        ...
        with col_bi_sub1:
            # Gráfica 2: Pie de Soporte Social (CON VERDE Y ROJO ACENTUADOS)
            casos_estres = df_bi[df_bi['Estres_Academico'] >= 7]
            fig_pie = px.pie(casos_estres, names="Apoyo_Social_Activo", 
                             title="Soporte Social en Estudiantes con Estrés Alto (>=7)",
                             color="Apoyo_Social_Activo",
                             color_discrete_map={"Sí": COLOR_SUCCESS, "No": COLOR_DANGER})
            fig_pie.update_layout(
                plot_bgcolor="white", 
                paper_bgcolor="white",
                font=dict(color="#1E293B", size=11),
                title_font=dict(color=COLOR_PRIMARY, size=14, family="Arial", weight="bold")
            )
            st.plotly_chart(fig_pie, use_container_width=True)

        with col_bi_sub2:
            # Gráfica 3: Densidad de Co-ocurrencia (CON ESCALA ESTÉTICA 'MINT' O 'VIRIDIS')
            fig_trend = px.density_heatmap(df_bi, x="Puntuacion_Ansiedad_GAD7", y="Puntuacion_Depresion_PHQ9", 
                                           ...
                                           labels={"Puntuacion_Ansiedad_GAD7": "Ansiedad (GAD-7)", "Puntuacion_Depresion_PHQ9": "Depresión (PHQ-9)"})
