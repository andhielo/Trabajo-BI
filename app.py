...
# Columnas simuladas provenientes de la API IoT de wearables (condición física)
COLUMNAS_IOT = ['FC_Promedio_BPM', 'SpO2_Porcentaje', 'Pasos_Diarios_Promedio', 'Calidad_Sueno_IoT', 'Nivel_Actividad_Fisica']
...
def grafico_proyeccion_sueno_estres(df):
    """Proyección en barras + línea de tendencia (sin puntos dispersos):
    Estrés académico promedio por rango de horas de sueño, con línea de tendencia lineal."""
    labels = ["<5h", "5-6h", "6-7h", "7-8h", "8-9h", "9h+"]
    bins = [0, 5, 6, 7, 8, 9, 24]
    df = df.copy()
    df['Rango_Sueno'] = pd.cut(df['Horas_Sueno_Promedio'], bins=bins, labels=labels, right=False)
    agrupado = df.groupby('Rango_Sueno', observed=True)['Estres_Academico'].mean().reindex(labels)

    x_idx = np.arange(len(labels))
    y_vals = agrupado.values.astype(float)
    validos = ~np.isnan(y_vals)

    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=labels, y=y_vals, name="Estrés promedio",
        marker_color=COLOR_PRIMARY, opacity=0.85
    ))

    if validos.sum() >= 2:
        coef = np.polyfit(x_idx[validos], y_vals[validos], 1)
        tendencia = np.polyval(coef, x_idx)
        fig.add_trace(go.Scatter(
            x=labels, y=tendencia, name="Tendencia (proyección)",
            mode="lines+markers", line=dict(color=COLOR_DANGER, width=3),
            marker=dict(size=8, color=COLOR_DANGER)
        ))

    fig.update_layout(yaxis_title="Estrés Académico (1-10)", xaxis_title="Horas de sueño promedio")
    return fig


def grafico_proyeccion_riesgo_ciclo(df):
    """Línea de tendencia: % de riesgo alto proyectado por ciclo académico."""
    riesgo_ciclo = df.groupby('Ciclo_Academico').apply(
        lambda g: (g['Riesgo_Predicho_IA'] == 'Riesgo Alto').mean() * 100
    ).reset_index(name='Pct_Riesgo_Alto').sort_values('Ciclo_Academico')

    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=riesgo_ciclo['Ciclo_Academico'], y=riesgo_ciclo['Pct_Riesgo_Alto'],
        name="% Riesgo Alto", marker_color=COLOR_WARNING, opacity=0.55
    ))
    if len(riesgo_ciclo) >= 2:
        coef = np.polyfit(riesgo_ciclo['Ciclo_Academico'], riesgo_ciclo['Pct_Riesgo_Alto'], 1)
        tendencia = np.polyval(coef, riesgo_ciclo['Ciclo_Academico'])
        fig.add_trace(go.Scatter(
            x=riesgo_ciclo['Ciclo_Academico'], y=tendencia, name="Tendencia (proyección)",
            mode="lines", line=dict(color=COLOR_DANGER, width=3)
        ))
    fig.update_layout(yaxis_title="% Riesgo Alto", xaxis_title="Ciclo académico")
    return fig


def simular_datos_iot_api(df_base):
    """
    Simula la recolección de datos desde 'BioPulse API', un servicio IoT de wearables
    que reporta condición física del estudiante (frecuencia cardiaca, saturación de
    oxígeno, actividad y calidad de sueño objetiva medida por sensor).
    Se correlaciona levemente con el estrés académico ya registrado, para que la
    simulación sea coherente con el resto del dataset.
    """
    df_base = df_base.drop(columns=[c for c in COLUMNAS_IOT if c in df_base.columns]).reset_index(drop=True)
    n = len(df_base)
    rng = np.random.default_rng()  # semilla variable → cada sincronización trae una "nueva lectura"

    estres_base = df_base['Estres_Academico'].values if 'Estres_Academico' in df_base.columns else rng.integers(1, 11, n)

    fc = np.clip(60 + estres_base * 2.2 + rng.normal(0, 6, n), 55, 135).round(0).astype(int)
    spo2 = np.clip(rng.normal(97, 1.3, n), 90, 100).round(1)
    pasos = np.clip(rng.normal(7000, 2600, n), 500, 18000).round(0).astype(int)
    calidad_sueno_iot = np.clip(rng.normal(70, 15, n) - (estres_base * 1.6), 10, 100).round(0).astype(int)
    nivel_actividad = pd.cut(pasos, bins=[-1, 5000, 9000, 100000], labels=["Bajo", "Moderado", "Alto"])

    iot_df = pd.DataFrame({
        'FC_Promedio_BPM': fc,
        'SpO2_Porcentaje': spo2,
        'Pasos_Diarios_Promedio': pasos,
        'Calidad_Sueno_IoT': calidad_sueno_iot,
        'Nivel_Actividad_Fisica': nivel_actividad
    })

    return pd.concat([df_base, iot_df], axis=1)
...
st.markdown(f"""
    ...
    /* ===== FILTROS: selectbox, multiselect, radio — forzar tema claro legible ===== */
    ...
    /* Selectbox / Multiselect: caja de control */
    ...
    div[data-baseweb="select"] svg {{ fill: {COLOR_MUTED} !important; }}
    ...
    [data-testid="stFileUploaderDropzone"] * {{ color: {COLOR_MUTED} !important; }}
    </style>
""", unsafe_allow_html=True)
...
# --- PASO 1: FUENTES DE DATOS ---
    ...
    # ==========================================
    # APARTADO: INFORMACIÓN DE API IoT RECOLECTADA (BioPulse API)
    # ==========================================
    st.write("")
    iot_sincronizado = hay_datos and all(c in df_raw.columns for c in COLUMNAS_IOT)
    estado_color = COLOR_SUCCESS if iot_sincronizado else COLOR_WARNING
    estado_texto = "🟢 Sincronizada" if iot_sincronizado else "🟡 En espera de sincronización"

    st.markdown(f"""
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
        <h3 style="color:{COLOR_TEXT}; margin:0;">📡 Información de API IoT Recolectada — <span style="color:{COLOR_PRIMARY};">BioPulse API</span></h3>
    ...
    with st.container(border=True):
        st.markdown(f"""
        <p style="color:{COLOR_MUTED}; font-size:13px; margin-bottom:2px;">
        Fuente complementaria simulada: dispositivos <b>wearables</b> (smartbands) que reportan condición física
        del estudiante en tiempo real — frecuencia cardiaca, saturación de oxígeno, nivel de actividad y calidad
        de sueño objetiva medida por sensor — para enriquecer el análisis de salud mental.
        </p>
        <p style="color:{COLOR_MUTED}; font-size:12px; margin-bottom:14px;">
        Endpoint simulado: <code>GET https://api.biopulse.io/v1/wearables/vitals</code>
        </p>
        """, unsafe_allow_html=True)

        if not hay_datos:
            st.info("Carga o genera un dataset primero para poder sincronizar los sensores IoT por estudiante.")
        else:
            col_iot_btn, _ = st.columns([1, 2])
            with col_iot_btn:
                label_btn = "🔄 Re-sincronizar lectura IoT" if iot_sincronizado else "📡 Sincronizar datos IoT (BioPulse API)"
                if st.button(label_btn, use_container_width=True):
                    st.session_state['dataset_crudo'] = simular_datos_iot_api(df_raw)
                    st.rerun()

            if iot_sincronizado:
                st.write("")
                m1, m2, m3, m4 = st.columns(4)
                metric_card(m1, "FC promedio", int(df_raw['FC_Promedio_BPM'].mean()), COLOR_DANGER, suffix=" bpm", icon="❤️")
                metric_card(m2, "SpO2 promedio", round(df_raw['SpO2_Porcentaje'].mean(), 1), COLOR_ACCENT, suffix="%", icon="🫁")
                metric_card(m3, "Pasos diarios prom.", int(df_raw['Pasos_Diarios_Promedio'].mean()), COLOR_SUCCESS, icon="🚶")
                metric_card(m4, "Calidad de sueño IoT", int(df_raw['Calidad_Sueno_IoT'].mean()), COLOR_PRIMARY, suffix="/100", icon="📶")

                st.write("")
                st.markdown(f"<p style='color:{COLOR_MUTED}; font-size:12px;'>Muestra de lecturas recolectadas por BioPulse API:</p>", unsafe_allow_html=True)
                cols_preview = (['Hash_ID'] if 'Hash_ID' in df_raw.columns else []) + COLUMNAS_IOT
                st.dataframe(df_raw[cols_preview].head(10), use_container_width=True)
...
# --- PASO 5: CAPA DE IA ÉTICA ---
    ...
    if not hay_datos:
        ...
        with col_ia2:
            fig_sueno_estres = grafico_proyeccion_sueno_estres(df_pred)
            st.plotly_chart(aplicar_tema_grafico(fig_sueno_estres, "Estrés Académico según Horas de Sueño (proyección)"), use_container_width=True)

        fig_riesgo_ciclo = grafico_proyeccion_riesgo_ciclo(df_pred)
        st.plotly_chart(aplicar_tema_grafico(fig_riesgo_ciclo, "Proyección de Riesgo Alto por Ciclo Académico"), use_container_width=True)
        ...
        if len(df_bi) > 0:
            ...
            with col_b:
                fig_box = px.box(df_bi, x='Universidad', y='Puntuacion_Depresion_PHQ9', color='Universidad',
                                  color_discrete_sequence=[COLOR_PRIMARY, COLOR_ACCENT, COLOR_SUCCESS, COLOR_WARNING, COLOR_DANGER, COLOR_MUTED])
