...
if 'dataset_crudo' not in st.session_state:
    # Generar datos base simulados por defecto si no suben uno
    np.random.seed(42)
    n_samples = 1000
    ...
    st.session_state['dataset_crudo'] = pd.DataFrame({
        ...
        'Distrito': np.random.choice(["Los Olivos", "San Martín de Porres", "Comas", "Carabayllo", "Independencia"], n_samples),
        'GAD7_Ansiedad': np.random.randint(0, 22, n_samples),
        'PHQ9_Depresion': np.random.randint(0, 28, n_samples),
        'Horas_Sueno': np.round(np.random.uniform(4.0, 9.0, n_samples), 1),
        'Estres_Academico': np.random.randint(1, 11, n_samples),
        'Apoyo_Social_Activo': np.random.choice(["Sí", "No"], n_samples, p=[0.7, 0.3])
    })
...
# ----------------- PASO 1: FUENTES DE DATOS -----------------
    ...
    st.markdown(f"<div style='background-color: #ECFDF5; padding: 12px; border-radius: 6px; margin: 15px 0; border-left: 5px solid #10B981; color: #065F46; font-weight: bold;'>Registros leídos: {len(df_active)} | Válidos: {len(df_active)}</div>", unsafe_allow_html=True)
...
# ----------------- PASO 2: STAGING AREA -----------------
    ...
    # Reemplazar la columna identificadora con HASH simulando cifrado ético
...
# ----------------- PASO 3: PROCESO ETL -----------------
    ...
    with st.spinner("Ejecutando pipeline de limpieza y transformación de datos..."):
        ...
        df_etl['Nivel_Ansiedad'] = pd.cut(df_etl['GAD7_Ansiedad'], bins=[-1, 4, 9, 14, 21], labels=["Mínimo", "Leve", "Moderado", "Severo"])
        df_etl['Nivel_Depresion'] = pd.cut(df_etl['PHQ9_Depresion'], bins=[-1, 4, 9, 14, 19, 27], labels=["Mínimo", "Leve", "Moderado", "Moderado-Severo", "Severo"])

    st.success("✅ ¡Transformación completada con éxito!")
    st.dataframe(df_etl[['Hash_ID', 'Universidad', 'Nivel_Ansiedad', 'Nivel_Depresion', 'Horas_Sueno', 'Estres_Academico']].head(10), use_container_width=True)
...
# ----------------- PASO 4: DATA WAREHOUSE -----------------
    ...
    # Gráfico del Modelo Físico/Lógico simulando tus esquemas
    fig = go.Figure()
    fig.add_annotation(x=0.5, y=0.5, text="FACT_SALUD_MENTAL<br><br>PK_Evaluacion<br>FK_Estudiante (Hash)<br>FK_Universidad<br>FK_Tiempo<br>Metrica_GAD7<br>Metrica_PHQ9<br>Metrica_HorasSueno",
                       showarrow=False, bbox=dict(boxstyle="round", fc="#EFF6FF", ec="#3B82F6", lw=2), font=dict(size=14, color="#1E3A8A"))
    fig.add_annotation(x=0.1, y=0.5, text="DIM_ESTUDIANTE<br><br>PK_Estudiante<br>Edad<br>Sexo<br>Distrito_Sede",
                       showarrow=False, bbox=dict(boxstyle="round", fc="#F8FAFC", ec="#64748B", lw=1), font=dict(size=12, color="#0F172A"))
    fig.add_annotation(x=0.9, y=0.5, text="DIM_UNIVERSIDAD<br><br>PK_Universidad<br>Nombre_Sede<br>Abreviatura",
                       showarrow=False, bbox=dict(boxstyle="round", fc="#F8FAFC", ec="#64748B", lw=1), font=dict(size=12, color="#0F172A"))
...
# ----------------- PASO 5: CAPA DE IA ÉTICA -----------------
    ...
    # Generar probabilidades realistas en base a GAD7 y PHQ9
    score_riesgo = (df_pred['GAD7_Ansiedad'] * 0.4 + df_pred['PHQ9_Depresion'] * 0.4 + df_pred['Estres_Academico'] * 0.2) / 25
    df_pred['Prob_Riesgo'] = np.clip(score_riesgo, 0.05, 0.98).round(3)
    df_pred['Precio_Predicho_IA'] = np.where(df_pred['Prob_Riesgo'] > 0.65, "Riesgo Alto", np.where(df_pred['Prob_Riesgo'] > 0.35, "Riesgo Moderado", "Riesgo Bajo"))

    col_ia1, col_ia2 = st.columns(2)
    with col_ia1:
        # Gráfica de distribución de probabilidades de riesgo
        fig_prob = px.histogram(df_pred, x="Prob_Riesgo", color="Precio_Predicho_IA", 
    ...
    with col_ia2:
        # Gráfica de estrés académico vs horas de sueño
        fig_scatter = px.scatter(df_pred, x="Horas_Sueno", y="Estres_Academico", color="Precio_Predicho_IA",
    ...
    st.dataframe(df_pred[['Hash_ID', 'Universidad', 'Prob_Riesgo', 'Precio_Predicho_IA']].head(10), use_container_width=True)
...
# ----------------- PASO 6: CAPA SEMÁNTICA & KPIS -----------------
    ...
    # Filtros interactivos al igual que tus capturas
    ...
    if d_sel:
        df_filtered = df_filtered[df_filtered['Distrito'].isin(d_sel)]
    ...
    with kpi2:
        st.markdown(f'<div class="metric-label">GAD-7 Promedio (Ansiedad)</div><div class="metric-value">{df_filtered["GAD7_Ansiedad"].mean():.2f}/21</div>', unsafe_allow_html=True)
    with kpi3:
        st.markdown(f'<div class="metric-label">PHQ-9 Promedio (Depresión)</div><div class="metric-value">{df_filtered["PHQ9_Depresion"].mean():.2f}/27</div>', unsafe_allow_html=True)
    with kpi4:
        st.markdown(f'<div class="metric-label">Promedio de Sueño</div><div class="metric-value">{df_filtered["Horas_Sueno"].mean():.2f} hrs</div>', unsafe_allow_html=True)
...
# ----------------- PASO 7: VISUALIZACIÓN BI -----------------
    ...
    with col_bi_f2:
        # Gráfica 1: Top 10 distritos por nivel de ansiedad promedio
        dist_ans = df_bi.groupby('Distrito')['GAD7_Ansiedad'].mean().reset_index().sort_values(by='GAD7_Ansiedad', ascending=False)
        fig_bar = px.bar(dist_ans, x='Distrito', y='GAD7_Ansiedad', title="Nivel de Ansiedad GAD-7 Promedio por Distrito",
        ...
        with col_bi_sub1:
            # Gráfica 2: Distribución de Soporte Social en casos de alto estrés
        ...
        with col_bi_sub2:
            # Gráfica 3: Evolución o densidad de ansiedad vs depresión
            fig_trend = px.density_heatmap(df_bi, x="GAD7_Ansiedad", y="PHQ9_Depresion", 
