...
# Rutas relativas para que funcione en Streamlit Cloud y en local
...
def cargar_datos_demo(ruta):
    if os.path.exists(ruta):
        ...
        st.error(f"Archivo no encontrado: {ruta}. Asegúrate de que esté en la misma carpeta que App.py")
        return None
...
def mostrar_pantalla_general():
    ...
    st.info("El objetivo es **detectar patrones de riesgo psicoemocional** (estrés, ansiedad) de forma ética y anónima para apoyar la toma de decisiones en Bienestar Universitario.")
...
def mostrar_etl():
    ...
    with st.spinner("Procesando transformaciones..."):
        ...
        # Ingeniería de variables (Cálculo de Riesgo)
        req_cols = ["nivel_estres", "nivel_ansiedad", "nivel_animo", "apoyo_social"]
        if all(c in df.columns for c in req_cols):
            # Normalización y cálculo del índice (0-10)
            df["indice_riesgo"] = (
...
def mostrar_data_warehouse():
    ...
    st.markdown("""
    <div class="kpi-card">
    <b>Modelo Estrella Logico:</b> <code>FACT_SALUD_MENTAL</code> rodeada por <code>DIM_ESTUDIANTE</code> y <code>DIM_TIEMPO</code>.
    </div><br>
    """, unsafe_allow_html=True)
...
def mostrar_ia():
    ...
    if st.button("🧠 Entrenar Modelo Random Forest", type="primary"):
        ...
        imp_df = pd.DataFrame({"Variable": features, "Importancia": clf.feature_importances_}).sort_values("Importancia", ascending=True)
        fig = px.bar(imp_df, x="Importancia", y="Variable", orientation='h', title="Importancia de Variables (Transparencia)")
        st.plotly_chart(fig)
...
def mostrar_bi():
    ...
    if "nivel_riesgo" in df.columns:
        ...
        with c1:
            fig1 = px.pie(df, names="nivel_riesgo", title="Distribución de Riesgo Psicoemocional",
                          color="nivel_riesgo", color_discrete_map={"Bajo": "#2ecc71", "Medio": "#f39c12", "Alto": "#e74c3c"})
            st.plotly_chart(fig1, use_container_width=True)
            
        with c2:
            if "universidad" in df.columns:
                fig2 = px.histogram(df, x="universidad", color="nivel_riesgo", barmode="group",
                                    title="Riesgo por Universidad", color_discrete_map={"Bajo": "#2ecc71", "Medio": "#f39c12", "Alto": "#e74c3c"})
